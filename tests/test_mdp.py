...
from openmdp.agents import OptimalMDPPolicyAgent
from openmdp.scenarios import WindyGridWorldMDP, CliffWalkMDP
...
from tests.test_pomdp import AccumulatedRewardMetric


class MDPSolverTests(TestCase):

    @staticmethod
    def validate_mdp_optimal_policy(mdp, optimal_steps_to_solve, episodes=100):
        agent = OptimalMDPPolicyAgent(mdp)
    ...
    def test_windy_grid_world_optimal_policy(self):
        self.validate_mdp_optimal_policy(WindyGridWorldMDP(), 15, episodes=100)

    def test_cliff_walk_optimal_policy(self):
        self.validate_mdp_optimal_policy(CliffWalkMDP(), 13, episodes=100)
    ...
    def test_duo_navigation_mdp(self):
        metric = AccumulatedRewardMetric()
        mdp = DuoNavigationMDP()
        agent = OptimalMDPPolicyAgent(mdp)
        horizon = 75
        min_reward = 0.00
        runner = TimestepRunner(horizon, agent, mdp, [metric])
        runner.run()
        assert metric.result() > min_reward, f"MDP DuoNavigationMDP was not solvable (full observable DuoNavigationMDP) (reward={metric.result()}"
