import numpy as np
from openmdp import MarkovDecisionProcess as MDP
...
class WindyGridWorldMDP(MDP):
    ...
    def __init__(self, rows=7, columns=10,
        ...
        P = self.setup_transition_probabilities(X, num_actions, wind)
        R = self.setup_rewards(X, num_actions, self._goal_state)
        miu = np.zeros(len(X))
        miu[self.state_index_from(X, self._start_state)] = 1.0

        super(WindyGridWorldMDP, self).__init__("WindyGridWorldMDP-v0",
                                                X, num_actions, P, R, discount_factor, miu,
    ...
    @staticmethod
    def setup_transition_probabilities(states, num_actions, wind):
    ...
    @staticmethod
    def setup_rewards(states, num_actions, goal_state):
        X = len(states)
        A = num_actions
        R = np.full((X, A), -1.0)
